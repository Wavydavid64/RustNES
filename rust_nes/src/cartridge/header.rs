...
const CHARACTER_RAM_IF_NO_ROM: usize = 8 * 1024;
...
pub fn process_header(header: &[u8]) -> Result<Metadata, Error> {
    ...
    let character_rom_bytes = CHARACTER_ROM_UNIT * (header[5] as usize);
    let character_ram_bytes = if character_rom_bytes == 0 {
        CHARACTER_RAM_IF_NO_ROM
    } else {
        0
    };
    ...
    let mut metadata = Metadata {
        ...
        character_rom_bytes: character_rom_bytes,
        character_ram_bytes: character_ram_bytes,
        ..Default::default()
    };
...
fn process_byte_8(metadata: &mut Metadata, byte: u8) {
    metadata.program_ram_bytes = PROGRAM_RAM_UNIT * std::cmp::max(byte as usize, 1)
}
